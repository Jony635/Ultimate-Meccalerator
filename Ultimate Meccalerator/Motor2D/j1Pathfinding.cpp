...
int j1PathFinding::CreatePath(const iPoint& origin, const iPoint& destination, int filter)
{
	...
	if (IsWalkable(origin) && IsWalkable(destination))
	{
		...
		PathList open;
		PathList close;
		PathNode startNode(1, origin.DistanceTo(destination), origin, nullptr);
		...
		while (open.list.count() > 0)
		{
			...
			for (p2List_item<PathNode>* itnode = neighbours.list.start; itnode != nullptr; itnode = itnode->next)
			{
				if (close.Find(itnode->data.pos) != nullptr)
					continue;
				p2List_item<PathNode>* pathnode = open.Find(itnode->data.pos);
				if (pathnode != nullptr)
				{
