...
{
	if (App->entities->player->position.x <= (-1 * (camera.x - (camera.w / 2))) - App->map->data.tile_width && camera.x < 0/*&&*/
		/*(App->input->GetKey(SDL_SCANCODE_A) == KEY_DOWN || App->input->GetKey(SDL_SCANCODE_A) == KEY_REPEAT)*/)
	{
		App->map->UpdateLayers("left", dt);
		fcamera.x += App->entities->player->speed_x * 75 * dt;
		camera.x = fcamera.x;
	}
	else if (App->entities->player->position.x >= -1 * (camera.x - (camera.w / 2)) + App->map->data.tile_width && -1 * (camera.x - camera.w) <= App->map->data.width*App->map->data.tile_width /*&&*/
	...
	camera.y = App->entities->player->position.y * -1 + 500;
}